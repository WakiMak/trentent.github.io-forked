...
author: trententtye
layout: revision
guid: http://theorypc.ca/2016/06/13/1488-revision-v1/
permalink: /2016/06/13/1488-revision-v1/
---
I was asked to justify upgrading our PVS vDisks to VHDX from VHD. Â There are a few &#8216;feature&#8217; / technical reasons:

  1. Use of native tools to mount/compress VHDX from Windows Server 2012. Â [VHD files created with 16MB block sizes require a custom Citrix tool](http://theorypc.ca/2014/05/21/how-to-convert-a-16mb-block-size-pvs-vhd-to-a-2mb-pvs-vhd-file-using-hyper-v/) which does not do compress.
...
So I know VHDX is supposed to perform better but I was curious by how much. Â Apparently, the modification Citrix made to the VHD format to a 16MB block size is &#8216;4K&#8217; aligned as well.
...
I set my target devices to the different vDisks and set the &#8216;Cache to RAM&#8217; feature to 4096MB. Â Ideally, all writes should be to RAM but this will still tax the filesystem.

And what is the performance between the two? Â I used the DiskSpd utility from Microsoft to measure the differences.
...
How much of this is the tools vs the format? Â I&#8217;m not sure, I didn&#8217;t have the time to reverse image and upgrade a VHD to 7.7. Â Regardless, the combination of upgrading to 7.7 from 7.1SP2 AND the VHDX format brought a dramatic boot time improvement and consistently faster disk speed.
