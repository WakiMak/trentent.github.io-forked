...
date: 2018-10-29T13:50:37-06:00
author: trententtye
layout: revision
guid: https://theorypc.ca/2018/10/29/2829-revision-v1/
permalink: /2018/10/29/2829-revision-v1/
---
## Challenge:
...
The challenges in this process occurs if the development of fixes/upgrade/enhancements spans multiple image &#8220;baking&#8221; sessions over a time period of weeks or months.Â It seems no matter how good you think your documentation is, if the development spans 10&#8217;s to 100&#8217;s of tweaks over sufficient time, something important will be lost in the chain.Â The process of enabling the change into production followed this process:<figure class="wp-block-image">
...
In the original design test and production were wholly separate and lived apart.Â The process of documenting and pushing changes from test into prod actually worked quite well until we started encountering the lengthy development/test change time scales, and numerous /different/ changes that occur across the different images.Â Copying from test into production was done a few times and painful results each time.Â The original image and design was not done to accommodate such a scenario so items that needed to be updated from test to prod were numerous and onerous.Â Scripts, registry, files, group policies&#8230;Â during some of these copies outages were incurred, again, because something was inevitably missed.Â At this point, trying to flush out all the different changes that would have to be done across 8 images and than maintaining the list was fairly daunting for our team.Â Having outages is a really, really bad thing.

### What&#8217;s wrong with 16 images?

Our images have branched from each other in various forms going back to that first image created around 2011.Â These images all have lineages 7 years old and with age comes problems.Â We have been finding that these images have had some corruption in some form or another.Â Whether it&#8217;s a registry key that&#8217;s lost its ACL or is completely corrupted, Windows Updates that fail to install, a file whose content is now garbage as opposed to clean text, or the complete inability to uninstall or upgrade some software packages.Â Thus far, none of these problems have been unsolvable, but the fact they are cropping with more regularity across all of our images is very worrisome.Â The time taken to resolve these issues as they occur is getting excessive and more common.Â When we looked at just updating these images for XenDesktop 7 we found we were unable to do so because XenApp 6.5 would not uninstall from some of them!
...
Because of a separation of test and production the amount of changes during the development stage targeting test servers/components/configs inevitable have something get lost/forgotten/missed.Â Copying test images to production is untenable in this process.Â So many hard-coded pieces embedded within the test images or configurations that need to be updated that when we \*have\* had to a test to prod copy things break and a lot of effort goes into tracking these additional hard-coded changes.

All of these issues exist within just a single OS, with a single version of Citrix.Â The future will be multi-OS with multiple versions of the Citrix VDA.Â Our current design is unfeasible if the various OS&#8217;s start encountering similar, unique image requirements.Â And it may be naive to think we can avoid it.Â But I believe we can put forth a design to make this more manageable, more predictable and more stable.
