...
date: 2016-04-25T08:16:26-06:00
author: trententtye
layout: revision
guid: http://theorypc.ca/blog/2016/04/25/645-revision-v1/
permalink: /2016/04/25/645-revision-v1/
---
...
To determine the cause of the issue, I used the nicely revamped Event Viewer and looked at the BITS-Client logs. Â Which was a waste, nothing showed up there. Â I checked the WindowsUpdateClient log and nothing was there either. Â I then learned BITS uses WinHTTP when I was googling for this issue and saw there was a WinHTTP log file. Â (You may have to enable analytics and debug logs). Â I enabled the Diagnostics Log.
...
When reattempting to execute Windows Update I went back into the log and scanned through it. Â I found the following:
...
Windows update was going to the wrong server! Â The event viewer said it was going to wswsus02.XXXX.ab.ca. Â This was our old server address and we since replaced it with going directly to the IP of that server via GPO.
...
These did not appear to work however. Â But, we did try the following:  
esentutl /p %windir%securitydatabasesecedit.sdb /o  
Gpupdate /force

And I believe this worked. Â After running this command, WinHTTP reported that it was pulling the Windows Update from the Microsoft servers, not our WSUS server:
