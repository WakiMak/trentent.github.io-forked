...
author: trententtye
layout: revision
guid: http://theorypc.ca/2018/04/30/2744-revision-v1/
permalink: /2018/04/30/2744-revision-v1/
---
...
If Spectre makes things use \*more\* resources, but the speed is the same, then the impact is only on capacity. Â Example: a task that took 5 seconds at 5% CPU utilization now takes 10% CPU utilization. Â In this scenario, the performance should be identical but your capacity is now halved.
...
The top 3 results are Spectre enabled machines, the bottom 3 are without the patch. Â The results are not good. Â We are seeing a 200% **speed** impact in this metric.
...
The component that took the largest hit is Group Policy. Â Again, ControlUp can drill down into this component.
...
All group policy preference components take a 200% hit. Â The Group Policy Preferences functions operate by pulling down an XML file from the SYSVOL store, reading the XML file, than applying whatever resultant set of policies finds applicable. Â In order to trace down further to find more differences, I logged into each type of machine, one with Spectre and one without, and started a Process Monitor trace. Â Group Policy is applied via the Group Policy service, which a seperate instance of the svchost.exe. Â The process can be found via Task Manager:
...
Setting ProcMon to filter only on that PID we can begin to evaluate the performance. Â I relogged in with procmon capturing the logon.
...
So it definitely appears we need to look at the registry actions. Â One of the cool things about Procmon is you can set a filter on your trace and open up the summaries and it will show you only the objects in the filter. Â I set a filter for RegSetValue to see what the impact is for setting values in the registry:
...
We see roughly the same ratio of performance degradation, perhaps a little more so. Â As a further test I created a PowerShell script that will just measure creating 1000 registry values and test it on each system:
...
We can get a better feel for the separation in CPU utilization between Spectre enabled/disabled. Â We are seeing clearly higher utilization.
...
Very clearly we can see the hosts with Spectre applied consume more CPU resources, even coming close to consuming 100% of the CPU resources on the hosts. Â Smoothing out the data using moving averages reveals the gap in performance with more clarity.
...
The Intel Xeon E5-2660 v4 has a frequency of <u>26% less</u> than the older 2680.Â In order to overcome this handicap, the processor must have improved at a per-clock rate higher than the 26% frequency loss.Â [CPUBenchMark](https://www.cpubenchmark.net/) had the two processors with a single thread CPU score of _<u>1616</u>_ <u>for the Intel Xeon E5-2660 v4</u> and _<u>1657</u>_ <u>for the Intel Xeon E5-2680</u>.Â This put them close but after 4 years the 2680 was marginally faster.Â This has played out in our testing that the higher frequency processor is faster.Â Performance degradation for the two processors came out as such:
