...
author: trententtye
layout: revision
guid: http://theorypc.ca/2016/10/31/1789-revision-v1/
permalink: /2016/10/31/1789-revision-v1/
---
We are getting this error when trying to install KB3172605, which was re-released in September of this year. Â This patch was originally installed in August without issue but the re-release fails to install. Â The WindowsUpdate.log reports:
...
One of the nice things about Windows Update is it kicks up Windows Error Reporting immediately when it detects an error. Â So if you run procmon.exe you can find when the error occurs and work backwards only a little bit to, usually, find why it crashed.
...
The actual Windows Error Reporting kicked in at the check to see if it is disabled (DisableWerReporting). Â The last key it read before it crashed was:
...
It appears the key that is in error is missing some information. Â In order to fix this I&#8217;m going to another system and try to grab they values needed for identity and S256H. Â I don&#8217;t know how these keys are generated but I&#8217;m hoping they are generated by the file it&#8217;s referring to which \*should\* mean that a same or similar level patched system should have the same files and thus the same generated hash&#8217;s.

I went to another system and exported the values. Â In order to find the proper key, we find it is referenced by the previous key in the procmon trace:  
...
I exported out the key from the working system and had to edit it so the GUID matched the broken key&#8217;s values. Â I took the &#8216;working&#8217; registry:
...
And then added the last &#8220;c!&#8221; line by following these next steps:
  ...
  1. copied the line out:  
    &#8220;c!c4ebacc5355..93c10e6175a\_31bf3856ad364e35\_6.1.7601.23455_8459b72d1e2f2700&#8243;=hex:
  ...
  3. Starting at the ellipse, counted the number of characters from the &#8220;55..&#8221; to &#8220;93&#8242; underneath and created the value: <pre class="lang:default decode:true">"c!c4ebacc5355..93c10e6175a_31bf3856ad364e35_6.1.7601.23455_8459b72d1e2f2700"=hex:
