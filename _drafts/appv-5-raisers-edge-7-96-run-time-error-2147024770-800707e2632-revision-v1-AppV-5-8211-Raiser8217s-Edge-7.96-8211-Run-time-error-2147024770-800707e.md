...
author: trententtye
layout: revision
guid: http://theorypc.ca/2017/12/15/2632-revision-v1/
permalink: /2017/12/15/2632-revision-v1/
---
...
This error is giving us a few clues as to what might be happening.Â The most obvious message is the &#8220;8007007e&#8221; which is a standard windows error hex code which translates to:

8007007E = FileNotFoundException

So RE7.exe is not finding a file it&#8217;s looking for.Â With most AppV packages we can suss out the file it&#8217;s missing by using procmon and tracing for &#8220;FILE NOT FOUND&#8221; in the result field.Â Unfortunately, my searching for this message didÂ **_NOT_** result in finding a file that wasn&#8217;t resolved by another path.Â In other words, all files were accounted for.Â But the error message very clearly states that a file is missing.Â So the next step is to install the application locally and compare the launch differences between the local install and the AppV install.Â Again, process monitor makes this easy by using the &#8220;loaded modules&#8221; option.
...
The launches were identical, until the highlighted points.Â The local install, which works without issue, has an extra file that gets loaded.Â bbcor7.dll.

It appears, somehow, this file is getting loaded and registered dynamically on a local install, but this is not happening with the AppV install.Â I don&#8217;t see the file get searched for at all with the AppV install and tracing with procmon.Â However, executing aÂ regsvr32 /s &#8220;C:\Program Files (x86)\Blackbaud\The Raisers Edge 7\DLL\bbcor7.dll&#8221; during sequencingÂ _does_ do all the necessary work to register and allow RE7.exe to find and load the file in an AppV bubble.
