...
date: 2017-06-26T13:49:05-06:00
author: trententtye
layout: revision
guid: http://theorypc.ca/2017/06/26/2449-revision-v1/
permalink: /2017/06/26/2449-revision-v1/
---
We&#8217;ve been trying to upgrade our Citrix PVS tools to version 7.13 and had some issues. Â I need to reverse image so I can remove the software without dealing with theÂ in-place upgrade procedure. So I decided this would be a good time to update my reverse imaging process. Â This time I will only be using native tools built into the OS. Â With that, this is my new procedure for reverse imaging. Â It&#8217;s much faster and easier than my last process, but it does have a dependency. Â You must be running Windows 2012 or greater so that **dism.exe** is a part of the OS. Â This reverse imaging process is VMWare specific but the principals should apply to all hypervisors. Â The overview of the process is as follows:  
...
<img class="aligncenter size-full wp-image-2450" src="http://theorypc.ca/wp-content/uploads/2017/06/CreateHDD.gif" alt="" width="698" height="620" /> 
  2. Attach the newly created disk to your PVS server <div style="width: 940px;" class="wp-video">
      <video class="wp-video-shortcode" id="video-2470-89" width="940" height="674" preload="metadata" controls="controls"><source type="video/mp4" src="http://theorypc.ca/wp-content/uploads/2017/06/AttachVMDK2.mp4?_=89" /><a href="http://theorypc.ca/wp-content/uploads/2017/06/AttachVMDK2.mp4">http://theorypc.ca/wp-content/uploads/2017/06/AttachVMDK2.mp4</a></video>
    </div>

  3. Setup the disks on the PVS server. Â In my example, the G: drive is a mounted VHDX of the Citrix vDisk. Â I&#8217;m using DISM to capture an image file of this disk to a wim that I will apply to the local disk created in step 1. Â You will need to <span style="text-decoration: underline;"><strong>create a partition on the disk</strong></span> you made in step 1 and <span style="text-decoration: underline;"><strong>set it as active</strong></span> (if this is a MBR disk). Â Unfortunately, DISM does not do disk to disk imaging so this intermediary step is required. Â In my video here, the E: drive is the local disk that belongs to my target &#8216;build&#8217; server. Â In addition, you must fix the BCD store as it will point to the original partition, not your new target partition.  
...
bcdedit /store bcd /set {default} osdevice partition=e:</pre>
    ...
    <div style="width: 1024px;" class="wp-video">
      <video class="wp-video-shortcode" id="video-2470-90" width="1024" height="768" preload="metadata" controls="controls"><source type="video/mp4" src="http://theorypc.ca/wp-content/uploads/2017/06/2017-06-26_11-15-14.mp4?_=90" /><a href="http://theorypc.ca/wp-content/uploads/2017/06/2017-06-26_11-15-14.mp4">http://theorypc.ca/wp-content/uploads/2017/06/2017-06-26_11-15-14.mp4</a></video>
    </div>

  4. Remove your hard disk from the PVS virtual machine **<span style="text-decoration: underline;">without</span> deleting it**. Â Ensure the disk is attached to your BUILD target device. Â If you are booting up your BUILD target device from an ISO or PXE you may need to disable those features so that it boots from the hard disk. Â I also disconnect the network from the virtual machine because we will need to reset the computer account with AD.
  5. To reset the computer account with AD, you need to login to the system. Â The easiest method is if you know the local administrator password, log in with that and &#8220;rejoin&#8221; the domain. Â If not, disconnect the network then logon with an account that has recently logged onto the server. Â Cached credentials should be able to pass you through. Â Once you are in, connect the network and rejoin the domain. Â You can typically &#8220;rejoin&#8221; by changing the domain name to the NETBIOS name, or the reverse if the NETBIOS name is present <div style="width: 1130px;" class="wp-video">
      <video class="wp-video-shortcode" id="video-2470-91" width="1130" height="720" preload="metadata" controls="controls"><source type="video/mp4" src="http://theorypc.ca/wp-content/uploads/2017/06/JoinDomain.mp4?_=91" /><a href="http://theorypc.ca/wp-content/uploads/2017/06/JoinDomain.mp4">http://theorypc.ca/wp-content/uploads/2017/06/JoinDomain.mp4</a></video>
    </div>
    
    At this point you can do whatever work is required.</li> 
    
      * Once you are done with whatever work you need to do, set the target device to boot from <span style="text-decoration: underline;"><strong>hard disk</strong></span>, but keep your vDisk set as an option. Â This will attach the vDisk on boot as a secondary drive. Â This will allow us to image back to it. Â You need to ensure this vDisk is in <span style="text-decoration: underline;"><strong>PRIVATE mode</strong></span> OR has a <span style="text-decoration: underline;"><strong>Maintenance</strong> </span>version. Â This will set the vDisk to a Read/Write mode.  
...
<img class="aligncenter size-full wp-image-2468" src="http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard.png" alt="" width="258" height="417" srcset="http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard.png 258w, http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard-186x300.png 186w" sizes="(max-width: 258px) 100vw, 258px" />  
        And go through the imaging process.</p> <div style="width: 620px;" class="wp-video">
          <video class="wp-video-shortcode" id="video-2470-92" width="620" height="436" preload="metadata" controls="controls"><source type="video/mp4" src="http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard.mp4?_=92" /><a href="http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard.mp4">http://theorypc.ca/wp-content/uploads/2017/06/ImagingWizard.mp4</a></video>
        </div>
        
        I prefer to image to the vDisk volume (as seen in this process).</li> 
        
          * Done! Â Set the device to boot via the vDisk as opposed to Hard Disk, delete the local OS disk, boot up your vDisk and run any scripts to &#8216;finalize&#8217; your image.</ol> 
