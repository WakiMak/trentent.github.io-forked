...
##  <span style="color: #ff0000;">1 logon every 0.974 second. (1.02 logons per second)</span>
...
The peak access to Web Interface is <span style="text-decoration: underline;">2.10 launches per second</span>.
...
## <span style="color: #ff0000;"><strong>1 launch every 0.561 second. (1.78 launches per second)<br /> </strong></span>
...
So these numbers are _purely_ for logons through the web site.  But previous testing for PNA functionality showed that it's a \*much\* heavier traffic to Web Interface.  Why is the traffic heavier for PNA?  Because each time a user _logs on to a workstation_ it kicks in.  Whereas accessing the web interface requires some work the user must do, PNA is automatic.  It's also more convenient for users as they prefer to launch desktop/start menu icons.
