---
id: 2237
title: Citrix Storefront â€“ Adventures in customization â€“ Dynamically configure authentication based on group membership
...
Can I make this happen? Â To start I created a store with both ExplicitLogon (User name and password) and Domain pass-through authentication methods enabled.
...
And this is what theÂ **custom/script.js** looks like when we convert the flow to reality:
...
function explicitLogonCheck(data) {
    ...
    case "ExplicitLogon":
        ...
        break;
    case "Unknown":
        console.log ("Attempting to authenticate. Â If we cannot we will default to ExplicitLogon.");
		CTXS.Extensions.excludeAuthenticationMethod = function(authenticationMethod) {
			CTXS.trace("excluding auth:" + authenticationMethod + ":" + (authenticationMethod == CTXS.Authentication.Method.PASSTHROUGH));
			return (authenticationMethod == CTXS.Authentication.Method.PASSTHROUGH);
		};
...
It works perfectly! Â We can now define users to utilize ExplicitLogon via a user group. Â And if they are not a member of that user group it will do pass-through authentication. Â And even further to that, if they are NOT a member of the domain it will automatically redirect them to the Explicit Logon page!
