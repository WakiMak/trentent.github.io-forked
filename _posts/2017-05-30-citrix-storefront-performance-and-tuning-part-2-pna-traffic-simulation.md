...
![](/wp-content/uploads/2017/05/1stConnection.png)
  
The first time you connect via PNA

![](/wp-content/uploads/2017/05/Sebsequent_Connection.png)
  
Subsequent connections after icons have been cached.
  

![](/wp-content/uploads/2017/05/2NewApplications.png)
...
```plaintext
wcctl.exe -t PNA_WithAppLaunch.wcat -f settings.ubr -s CTXWI01 -p 80 -c 10 -v 60 -w 300 -u 300 -n 30
```
...
```shell
wcclient.exe Win7.bottheory.loca
```
...
```shell
.\Stress_StoreFrontPNA.ps1 -store http://storefront.bottheory.local/Citrix/Store/PNAgent/ -username trententtye -password c0mplexp@44 -domain BOTTHEORY.LOCAL -stressComponent "Launch Application"
...
.\Stress_StoreFrontPNA.ps1 -store https://webinterface.bottheory.local/Citrix/PNAgent/ -username trententtye -password c0mplexp@44 -domain BOTTHEORY.LOCA
```
...
.SYNOPSIS
   ...
   <a href="https://chentiangemalc.wordpress.com/2016/12/14/encrypt-password-in-ctx1-encoding-with-powershell/">Encrypt Password In CTX1 Encoding WithPowerShell</a>
