...
We started our investigation. Â The first thing we did was test to see if HTML5 Receiver works at all. Â We configured and enabled websockets on our & servers and then logged into the Storefront server directly, and internally. Â We were able to launch applications without issue.
...
I proceeded to spin up a home lab with & and a Netscaler configured for HTML5 Receiver and tried connecting. Â It worked flawlessly via the Netscaler. Â I enabled logging and took another look:
...
It was a fairly frustrating experience. Â I had tech's ask me to go to "Program Files\Citrix\Reciever" and get the receiver version (hint, hint, this does not exist with HTML5). Â I captured packets of the failure "in motion" and they told me, "it's not connecting to your & server". Â - Yup. Â That's the Problem.
...
So, to review what we learned:
  ...
  1. Connections via Netscaler to HTML5 reciever do NOT <span style="text-decoration: underline;">require</span>Â (but is possible) aÂ SSL connection on each target & device
