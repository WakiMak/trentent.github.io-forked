...
Why is there a Publishing Refresh event? Â It turns out, it can be triggered by a group policy refresh.
...
<table summary="table">
  <tr>
    ...
    <td colspan="1">
      Delete and recreate a registry value or key for computers or users. If the target is a registry value, the net result of theÂ <strong>Replace</strong>Â action is to overwrite all existing settings associated with the registry value. If the target is a registry key, the net result is to delete all values and subkeys in the key, leaving only a default value name with no data. If the registry value or key does not exist, then theÂ <strong>Replace</strong>Â action creates a new registry value or key.
    </td>
  </tr>
